...
import { GenerationOverlay } from "@/registry/new-york/generation-overlay";
...
export const GenerationOverlayDemo = () => {
 ...
 */
export const GenerationOverlaySeedsDemo = () => (
  <div className="grid w-full max-w-2xl grid-cols-2 gap-3 sm:grid-cols-3">
    {SEEDS.map((seed) => (
      <div className={cn(FRAME, "max-w-none border-border")} key={seed}>
        <GenerationOverlay pending prompt={seed} seed={seed} />
      </div>
    ))}
  </div>
);
...
export const GenerationOverlayFailureDemo = () => {
  const [pending, setPending] = useState(true);
