...
description: "A collapsible window onto a reasoning stream — fixed height, a mask that says the thought runs on below the fold, and a follow that lets the newest line rise into place instead of jumping the scroll."
---
...
```bash
npx shadcn@latest add https://startercn.vercel.app/r/thinking-block.json
```
...
fixed height (`208px` by default, `height` to change it) over a scroll container whose
scrollbar is hidden. The height is the block's one hard promise — a thought that runs to
forty paragraphs takes the same room as one that runs to two, and the answer underneath
never gets pushed down the page while the model is still thinking.
...
While the work is happening the header is a `LoadingState` row — the nine-square wave, the
