...
generated in an effect. Each mote's carrier spans the fill, so `100%` of its travel is the
fill's own width: nothing is measured in pixels, and the drift works the same whatever the
level's fill is worth.
