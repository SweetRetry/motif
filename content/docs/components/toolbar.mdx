...
```bash
npx shadcn@latest add https://startercn.vercel.app/r/toolbar.json
```
...
without reaching for it. Pass `displayLabel` and that word is pinned into the row — the
...
full name: the tooltip, and the accessible name. `displayLabel` is the shorthand the row
