...
import { AnimatedEdgeDemo } from "@/examples/animated-edge";

<ComponentPreview name="animated-edge">
  <AnimatedEdgeDemo />
</ComponentPreview>
...
```bash
npx shadcn@latest add https://startercn.vercel.app/r/animated-edge.json
```
...
```tsx
import { AnimatedEdge } from "@/components/ui/animated-edge";

export function Connector({ running }: { running: boolean }) {
  return <AnimatedEdge active={running} className="text-primary" />;
}
```
...
<ComponentSource
  name="animated-edge"
  src="registry/new-york/animated-edge.tsx"
  title="animated-edge.tsx"
/>
