...
The card reads its states in two tokens the base theme does not have: `warning` for a step
walked past, `success` for a checkmark. The registry item carries both, so `shadcn add`
adds them to your CSS — `:root`, `.dark` and `@theme`, the same three places
`styles/globals.css` keeps them here.
